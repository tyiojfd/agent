...
class BatchVideoCutter:
    ...
    def analyze_with_glm(self, segments, tag_dimensions):
        ...
        prompt = f"""分析这段鲁迅故里导览视频的完整讲解内容，识别不同的主题段落并确定切分点，同时为每个片段生成标签。
...
只返回JSON，不要其他内容。"""
    ...
    def generate_tags_with_glm(self, text, tag_dimensions):
        ...
        prompt = f"""根据以下鲁迅故里导览内容，从3个维度生成标签。

内容：
{text[:800]}

标签维度要求：
1. 主题：从以下选项中选择1-2个最符合的：{tag_dimensions['主题']}
2. 地点：根据内容推理具体地点（如"百草园"、"三味书屋"、"鲁迅故居"等），如果内容中没有明确地点则填"通用"
3. 季节：根据内容中的天气、植物、活动等线索推理季节（春天/夏天/秋天/冬天），如果无法判断则填"不确定"
